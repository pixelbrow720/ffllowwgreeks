...
**Phase:** M0–M9 backend + post-M9 hardening (A–H) + observability + production-readiness + deep review + production-proven hardening + **auth pivot to API keys** + **replay math unblocked** + **Sprint 1 frontend wired live** + **offline calibration tool** + **admin keys surface** — **✅ COMPLETE**

**Last session:** 2026-05-29 (PM, fan-out batch) — Multi-agent autonomous run that **closed bug #3, completed token migration, Frontend Sprint 1, offline calibration tool, and admin keys endpoints in parallel**. Six concrete deliverables landed in one session; backend test count grew 17 → 19 packages; web build clean; 0 NATS payload warns under 17-min unpaced replay.

1. **Bug #3 — `state.<sym>.gex` payload trim.** Top-`stateMaxStrikes`=64 strikes by `|DealerPos|` (deterministic tie-break on Expiry/Strike/Side) via new `topStrikesByDealerPos` helper in `cmd/compute/main.go` + `main_test.go` (5 cases). Wire format gains `strike_count_total` + `strike_count_returned` so consumers know they are looking at a concentration view. Smoke 2026-02-12 SPX 11:29→14:40 UTC unpaced: 1,515 rows in `dealer_state_1s`, avg spot 6813.01, 1,203/1,515 rows non-zero NetGEX (avg ≈ -54.5B), DPI 60→71, charm zone PEAK by 14:30. **Zero `nats: maximum payload exceeded` warns** during full 17-minute wall-clock run that previously spammed at 1 Hz.

2. **Web token migration.** 47 occurrences of `signal-up` / `signal-down` / `signal-warn` → `accent-long` / `accent-short` / `accent-warn` across 13 files in `web/src/components/`. Aligned with the new `tailwind.config.ts` accent palette per `CLAUDE.md` color discipline. `npm run build` + `npm run lint` clean. `signal-info` / `signal-pin` left alone — out of scope, separate cleanup.

3. **Frontend Sprint 1 — dashboard wired to live backend.** Generated typed contract (`web/src/lib/api/schema.ts` via `openapi-typescript`); typed REST client (`web/src/lib/api/client.ts`) with `Authorization: Bearer` + structured error; WS client (`web/src/lib/ws/client.ts`) with exp+jitter reconnect + 25s heartbeat probe + 40s stale watchdog + ref-counted channel subscribe; `useLiveSocket()` / `useChannel()` / `useSocketStatus()` hooks; shared snapshot store (`useSyncExternalStore`-backed) deduped across panels; in-memory accumulators for spot history + alert log. Migrated `Topbar`, `SpotChart`, `GEXProfile`, `KeyLevels`, `SignalLog` off mock data — layouts preserved exactly, skeleton + error states added. `npm run build` + `npm run lint` clean. WS auth via `?api_key=` query param (browser can't set headers on the upgrade); server currently ignores it, will be wired in a follow-up.

4. **Offline calibration tool — `cmd/calibrate`.** New 354-LOC binary that walks `dealer_state_1s` rows over a configurable window and emits a JSON config of empirical normalizers fit per symbol: `gex_norm` / `charm_flow_rate_norm` / `vanna_pressure_norm` (95th percentile of `|·|`); `charm_zone_boundaries` ([33rd, 66th] percentiles of raw signed `charm_velocity`); `pin_min_probability` (median of `pin_top_prob` where `pin_active=true`, omitted when no rows match). R-7 / NumPy-default percentile so analysts comparing in pandas see byte-identical numbers. CLI: `--from / --to / --symbol spx,ndx / --output / --postgres-url`. New `make calibrate` target builds + invokes against the 9-day archive (`2026-02-02 → 2026-02-13`) writing to `data/calibration/<date>.json` (gitignored). `cmd/calibrate/main_test.go` 264 LOC covering percentile math, empty-input handling, `pin_active=false` skip. `internal/dealer` and `cmd/compute` are NOT yet wired to consume the output — that's a deliberate follow-up so we can sanity-check the fit before flipping defaults.

5. **Admin keys endpoints — `internal/api/admin.go`.** Separate listener on loopback (`ADMIN_LISTEN_ADDR` default `127.0.0.1:9090`), gated by single shared-secret `ADMIN_TOKEN` via `subtle.ConstantTimeCompare`. Endpoints: `GET /admin/keys` (paginated, max 200, keyset cursor on id), `GET /admin/keys/{id}` (404 on unknown), `POST /admin/keys/{id}/revoke` (idempotent 204). Audit sink emits `admin.list` (INFO) + `admin.revoke` (WARN) for every operation; secret never returned, hash never returned, only an 8-hex SHA-256 prefix for visual disambiguation (`TestAdmin_NeverLeaksHashOrSecret` enforces this). Listener boots only when `ADMIN_TOKEN != ""` and `sharedPool != nil` so dev/test setups don't need it. Two-phase shutdown (5s admin → 15s public). `Store` interface gained `GetByID` + `ListPaged` implemented on both `PgStore` and `MemoryStore`. 12 new test cases under `internal/api/admin_test.go`, all green; existing `TestRESTSnapshot*` / `TestSimulate*` / `TestMiddleware_*` byte-for-byte identical.

6. **Build state across the board.** `go vet ./...` clean. `go test ./... -count=1 -timeout 120s` 19/19 packages green (gained `cmd/calibrate`). `npm run build` clean (web). `npm run lint` clean (web). All work uncommitted, ready for the user to commit.

7. **WS auth via `?api_key=` query param closed the Sprint 1 loose end.** `apikey.extractSecret` accepts the secret from the query string only on RFC 6455 WebSocket upgrades (Connection: Upgrade token list + Upgrade: websocket). Plain HTTP requests ignore the param so secrets don't leak into proxy logs / Referer / browser history. Headers win when both present. 7 new test cases in `internal/apikey/middleware_test.go`.

8. **Calibration consumer wired into `cmd/compute`.** New `--calibration-config <path>` flag on `cmd/compute/main.go` loads the JSON emitted by `cmd/calibrate` and applies it on startup. New `internal/dealer/calibration.go` parses the file, lower-cases symbol keys, and exposes `PreferredSymbol(map)` (SPX wins because the historical archive has full coverage; NDX fallback; zero-sample entries skipped). New `DPIScorer.SetThresholds(gex, charmFlow, vanna)` and `CharmClockClassifier.SetVelocityThresholds(weak, peak)` runtime setters validate inputs (positive, monotonic) so a malformed JSON cannot silently zero a production normalizer. The package-level `charmWeakVelocityCeiling` / `charmPeakVelocityFloor` constants moved to per-classifier struct fields seeded from new `DefaultCharmWeakVelocityCeiling` / `DefaultCharmPeakVelocityFloor` exports. Pin `min_probability` is parsed but not applied — pin engine has no clean trigger-probability gate; design call deferred for a follow-up. 12 new test cases in `internal/dealer/calibration_test.go`. `go test ./... -count=1` 19/19 packages green.

**Previous session:** 2026-05-29 (PM) — **Replay math now produces non-zero output for the first time.** Closed both deferred plumbing bugs from this morning's smoke-up: (1) `internal/replay/reader.go::scanTick` now reconstructs `Tick.FuturesContract` from `(symbol, ts)` via the new `internal/replay/futures.go::FrontMonthContract` helper (CME quarterlies H/M/U/Z, third-Friday rollover, SPX→ES, NDX→NQ). The `ticks` hypertable still has no `futures_contract` column — front-month derivation lives entirely in the reader, no schema migration needed. (2) `cmd/compute` now drives **all** time-sensitive math from a per-pipeline event-time high-water mark (`Pipeline.lastEventNs` atomic, fed off every tick's `TsEvent`) rather than wall-clock: `fillGreeks` TTE, DPI TTC decay, Charm Clock zone window, persisted `dealer_state_1s.ts`, and the published `state.spx.gex.ts_ns` all track historical event time during replay so SPX expiries in Feb 2026 don't collapse to T=0 against a 2026-05 wall clock. `DPIScorer` and `CharmClockClassifier` got `SetSessionBounds(start, end)` setters so the aggregator can rebuild the 09:30→16:00 ET session window per event-time day every iteration. Two adjacent fixes: (a) eager spot seeding from basis on the first futures tick so the IV solver doesn't waste its first thousand attempts against the hard-coded 5800 fallback when replay floods quotes; (b) bump `nc.Subscribe` pending limits to 8M msgs / 1 GiB so unpaced replay (publishes 1.8M ticks in ~17min wall) doesn't hit the 64K default and silently drop options ticks.

**Previous session:** 2026-05-29 (AM) — Python bridge load verified + smoke pipeline replay→compute→dealer_state_1s end-to-end. 211M ticks loaded across 9 trading days (2026-02-02 → 2026-02-12), 27 TimescaleDB chunks, 2.4 GB. Two P0 bugs surfaced + fixed during smoke: (1) **`bus.Publisher.subjectFor` did not handle `TickTypeOI`** — every OI tick was rejected silently, blocking `PositionTracker.SeedFromOI` and therefore the entire downstream state pipeline. Added `SubjectTickOI` + case in `subjectFor`. (2) **`PositionTracker` had no concurrency guard** despite documented "single-threaded by design" — `cmd/compute` actually drives writers (NATS callback) and readers (aggregator loop) on separate goroutines, causing a fatal `concurrent map iteration and map write` panic the moment positions accumulated. Added `sync.RWMutex` guarding all map access. After the fixes, smoke run materialized 250 rows in `dealer_state_1s` at 1 Hz cadence — **plumbing proven end-to-end** for the first time on real Databento ticks. All math fields (spot, NetGEX, DPI, etc.) currently 0 in the smoke output: see deferred bugs #1 + #2 above (closed in PM session).
