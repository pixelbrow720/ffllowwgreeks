...
def test_ws_stream_propagates_published_payload_within_100ms(monkeypatch) -> None:
    ...
    monkeypatch.setattr(snapshot_mod, "build_snapshot_payload", _empty_payload)
...
def test_ws_stream_revocation_closes_with_4401(monkeypatch) -> None:
    ...
    snapshot_mod.reset_snapshot_cache_for_tests()
    monkeypatch.setattr(
        stream_mod, "_revocation_check_interval_seconds", lambda: 0.05
    )
...
# ──────────────────────────────────────────────────────────────────────────
# Rev 8 ARCH-6 — global revocation watcher (consolidated, not per-conn)
# ──────────────────────────────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_arch6_register_subscriber_starts_global_watcher(monkeypatch) -> None:
    """First :func:`_register_revocation_subscriber` call must lazy-start
    the global watcher task."""
    stream_mod.reset_revocation_subscribers_for_tests()
    monkeypatch.setattr(
        stream_mod, "_revocation_check_interval_seconds", lambda: 60.0
    )
    ev = await stream_mod._register_revocation_subscriber("key-A")
    try:
        assert stream_mod._KEY_REVOCATION_TASK is not None
        assert not stream_mod._KEY_REVOCATION_TASK.done()
        assert ev in stream_mod._KEY_REVOCATION_SUBSCRIBERS["key-A"]
    finally:
        await stream_mod._deregister_revocation_subscriber("key-A", ev)
        await stream_mod.stop_global_revocation_watcher()


@pytest.mark.asyncio
async def test_arch6_global_watcher_signals_all_subscribers_on_revocation(
    monkeypatch,
) -> None:
    """One global poll fires events for every subscriber of the revoked key,
    while subscribers of OTHER keys remain untouched (per-key isolation).

    TQ-3 (Rev 9) — the previous version of this test made ``fake_check``
    return True for every key and asserted ``ev3.wait()`` resolved, which
    meant a regression that fanned out across keys would still pass.
    """
    stream_mod.reset_revocation_subscribers_for_tests()
    monkeypatch.setattr(
        stream_mod, "_revocation_check_interval_seconds", lambda: 0.01
    )

    async def fake_check(key_id):
        # Only key-X is revoked. key-Y subscribers MUST NOT fire.
        return key_id == "key-X"

    monkeypatch.setattr(stream_mod, "_check_key_revoked", fake_check)

    ev1 = await stream_mod._register_revocation_subscriber("key-X")
    ev2 = await stream_mod._register_revocation_subscriber("key-X")
    ev3 = await stream_mod._register_revocation_subscriber("key-Y")
    try:
        # ev1 + ev2 (both on key-X) MUST fire.
        await asyncio.wait_for(ev1.wait(), timeout=1.0)
        await asyncio.wait_for(ev2.wait(), timeout=1.0)
        # ev3 (key-Y) MUST NOT fire — give the watcher a few ticks then
        # assert. The poll interval is 10ms so 200ms is ~20 rounds.
        await asyncio.sleep(0.2)
        assert not ev3.is_set(), (
            "key-Y subscriber must not fire when only key-X is revoked"
        )
    finally:
        await stream_mod._deregister_revocation_subscriber("key-X", ev1)
        await stream_mod._deregister_revocation_subscriber("key-X", ev2)
        await stream_mod._deregister_revocation_subscriber("key-Y", ev3)
        await stream_mod.stop_global_revocation_watcher()


@pytest.mark.asyncio
async def test_arch6_deregister_drops_empty_buckets(monkeypatch) -> None:
    """Empty subscriber buckets must be removed so the watcher's
    ``watched`` snapshot stays bounded by the live connection set."""
    stream_mod.reset_revocation_subscribers_for_tests()
    monkeypatch.setattr(
        stream_mod, "_revocation_check_interval_seconds", lambda: 60.0
    )
    ev = await stream_mod._register_revocation_subscriber("key-Z")
    try:
        assert "key-Z" in stream_mod._KEY_REVOCATION_SUBSCRIBERS
    finally:
        await stream_mod._deregister_revocation_subscriber("key-Z", ev)
        await stream_mod.stop_global_revocation_watcher()
    assert "key-Z" not in stream_mod._KEY_REVOCATION_SUBSCRIBERS


@pytest.mark.asyncio
async def test_arch6_check_key_revoked_returns_true_for_inactive(monkeypatch) -> None:
    """``_check_key_revoked`` returns True when the row is inactive, missing,
    or expired — and False (do not kick) on transient DB errors."""

    class _Row:
        def __init__(self, *, is_active=True, expires_at=None, deleted_at=None):
    ...
    monkeypatch.setattr(stream_mod, "get_session_factory", lambda: factory_for(_Row(is_active=False)))
    assert await stream_mod._check_key_revoked("any") is True
    monkeypatch.setattr(stream_mod, "get_session_factory", lambda: factory_for(None))
    assert await stream_mod._check_key_revoked("any") is True
    monkeypatch.setattr(stream_mod, "get_session_factory", lambda: factory_for(_Row()))
    assert await stream_mod._check_key_revoked("any") is False


# ──────────────────────────────────────────────────────────────────────────
# Rev 12 BC-9 — WS snapshot frame discriminator
# ──────────────────────────────────────────────────────────────────────────


def test_ws_snapshot_frame_carries_type_discriminator(monkeypatch) -> None:
    """BC-9: every WS snapshot frame must include ``type: "snapshot"`` so
    consumers can dispatch uniformly across snapshot / tick / heartbeat /
    error frames. The field is additive to the existing ``data`` field —
    pre-Rev-12 clients keep working.
    """
    stream_mod.reset_ws_state_for_tests()
    notifier_mod.reset_stream_notifier_for_tests()
    _patch_session_factory(monkeypatch)
    ...
    class _FakeApiKey:
        id = "ws-bc9-discriminator"
        is_active = True
        expires_at = None
        allowed_symbols = ["SPXW"]

    async def _fake_auth(api_key, symbol, session):  # noqa: ARG001
        return _FakeApiKey() if api_key == "ak_bc9_fake" else None
    ...
    try:
        with client.websocket_connect(
            ...
            assert initial["type"] == "snapshot", (
                "BC-9: WS snapshot frame must carry type='snapshot'"
            )
            assert initial["symbol"] == "SPXW"
            assert "data" in initial  # additive — legacy field preserved.

            # A subsequent published frame (test path coerces a dict) must
            # also carry the discriminator.
            async def _publish() -> int:
                return await notifier_mod.get_stream_notifier().publish(
                    "SPXW", {"data": {"foo": 1}, "computed_at": None}
                )

            ws.portal.call(_publish)
            frame = ws.receive_json(mode="text")
            assert frame["type"] == "snapshot"
            assert frame["data"] == {"foo": 1}
    finally:
        client.close()


# ──────────────────────────────────────────────────────────────────────────
# Rev 12 BC-10 — WS error frame emission
# ──────────────────────────────────────────────────────────────────────────


def test_ws_error_frame_emitted_on_snapshot_prime_failure(monkeypatch) -> None:
    """BC-10: when the initial-snapshot prime fails, the WS handler must
    emit a structured ``WsErrorFrame`` ({type: "error", code: 503, ...})
    before continuing the pump loop. Pre-Rev-12 the failure was logged
    and silently swallowed; consumers waiting on ``type === "error"``
    saw nothing.
    """
    stream_mod.reset_ws_state_for_tests()
    notifier_mod.reset_stream_notifier_for_tests()
    ...
    async def _fake_auth(api_key, symbol, session):  # noqa: ARG001
        return _FakeApiKey() if api_key == "ak_bc10_fake" else None

    async def _boom_payload(session, symbol):  # noqa: ARG001
        raise RuntimeError("simulated prime failure")

    async def _boom_single_flight(session, symbol):  # noqa: ARG001
        raise RuntimeError("simulated single-flight prime failure")

    monkeypatch.setattr(stream_mod, "_authenticate_streaming_key", _fake_auth)
    # Patch BOTH the cache-miss path (build_snapshot_payload_single_flight)
    # AND the underlying build_snapshot_payload so any priming attempt
    # raises. The cache itself is wiped so we land on the cold-cache
    # branch.
    monkeypatch.setattr(snapshot_mod, "build_snapshot_payload", _boom_payload)
    monkeypatch.setattr(
        stream_mod,
    ...
    snapshot_mod.reset_snapshot_cache_for_tests()
    monkeypatch.setattr(stream_mod, "get_cached_snapshot", lambda _sym: None)

    client = _build_ws_test_client()
    plaintext = "ak_bc10_fake"
    try:
        with client.websocket_connect(
            "/v1/SPXW/stream", headers={"X-API-Key": plaintext}
        ) as ws:
            ...
            assert frame["type"] == "error", (
                "BC-10: prime failure must surface a structured error frame"
            )
            assert int(frame["code"]) == 503
