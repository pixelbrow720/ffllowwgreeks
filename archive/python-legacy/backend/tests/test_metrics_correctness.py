    ...
    for exp in (e1, e2):
        for K in (95, 100, 105):
            ...
            rows.append({"expiration": exp, "strike": K, "option_type": "P",
                         "oi": oi, "volume": 0})
    df = pd.DataFrame(rows)
    # DR-15: ``compute_max_pain`` now drops expired expiries by default;
    # pin ``today`` to ``_TODAY`` so the synthetic 2026-01-09 / 2026-01-16
    # expiries stay in scope under the new gate.
    out = compute_max_pain(df, fold_all=True, today=_TODAY)
    assert out.aggregate_strike == 100.0
    assert len(out.per_expiry) == 1
...
def test_metrics_produce_only_finite_floats_on_clean_chain():
    ...
    # DR-15: pin ``today`` to ``_TODAY`` so the synthetic future expiries
    # stay inside the new expired-row filter window.
    mp = compute_max_pain(df, today=_TODAY)
...
# NUM-3: zero_gamma must accept a session-aware τ override so an
# afternoon 0DTE-heavy chain reports a flip level computed at the real
# intraday τ instead of the 1-day calendar floor.


def test_zero_gamma_tau_override_shifts_flip_level_for_0dte_chain():
    """0DTE rows should have their τ overridden when ``tau_years`` is
    passed — different τ produces a different gamma profile and a
    different flip point."""
    spot = 5_800.0
    today = pd.Timestamp("2026-01-02")
    rows = []
    # Pure-put cluster below spot + pure-call cluster above spot, all 0DTE.
    for _ in range(30):
        rows.append({
            "strike": 5_700.0,
            "option_type": "P",
    ...
    df = pd.DataFrame(rows)
    intraday_tau = 5.0 / (365.0 * 24.0 * 60.0)  # 5 min to close
    zg_calendar = compute_zero_gamma(
        df, weight_col="oi", today=today, search_pct=0.05, n_points=801,
    )
    zg_intraday = compute_zero_gamma(
        df,
        weight_col="oi",
        today=today,
        search_pct=0.05,
        n_points=801,
        tau_years=intraday_tau,
    )
    # Both must be finite levels.
    assert zg_calendar is not None and math.isfinite(zg_calendar)
    assert zg_intraday is not None and math.isfinite(zg_intraday)
    # The two τ regimes produce *different* flip levels for the same
    # chain — proves the override actually flows through.
    assert abs(zg_calendar - zg_intraday) > 1e-3
