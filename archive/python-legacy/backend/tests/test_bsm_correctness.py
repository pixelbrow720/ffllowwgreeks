...
# ── 10. NUM-5: far-OTM 0DTE Greeks underflow safely to zero ─────────────────


def test_far_otm_0dte_gamma_does_not_leak_nan_or_inf() -> None:
    """At τ=15min, σ=0.50, K/S=1.20 → d1 ≈ 80, ``norm.pdf`` underflows.

    Without the d1 clip, the wing strike returns 0.0 from the IEEE
    underflow path. With the clip, the value is explicitly 0.0 and the
    wing OI no longer silently contaminates downstream aggregations
    when extreme inputs combine with NaN propagation. Regression: a
    deep-OTM strike must yield finite-zero, not NaN/inf, in vector
    inputs that mix near-ATM and wing strikes.
    """
    S = 5_800.0
    tau = 15.0 / (365.0 * 24.0 * 60.0)
    sigma = 0.50
    K = np.array([5_800.0, 6_000.0, 6_500.0, 7_000.0])  # ATM → deep OTM

    g = np.asarray(bsm.gamma(S, K, tau, sigma, r=0.05), dtype=float)
    assert np.all(np.isfinite(g))
    # ATM strike must still be positive; wings must return clean 0.0.
    assert g[0] > 0
    assert g[-1] == 0.0


def test_bsm_gamma_clip_increments_counter() -> None:
    """The module-level ``_clipped_count`` is incremented for any row
    whose ``|d1|`` blows past the clip limit. Ops use this to detect
    when the wings are being silently truncated by the BSM math."""
    S = 5_800.0
    tau = 15.0 / (365.0 * 24.0 * 60.0)
    sigma = 0.50
    before = bsm._clipped_count
    bsm.gamma(S, np.array([7_000.0]), tau, sigma, r=0.05)
    assert bsm._clipped_count > before


def test_bsm_charm_no_nan_inf_leak_on_deep_otm_chain() -> None:
    """Charm on a deep-OTM 0DTE chain must not introduce NaN/inf into
    downstream aggregations even when ``norm.pdf(d1)`` underflows."""
    S = 5_800.0
    tau = 15.0 / (365.0 * 24.0 * 60.0)
    sigma = 0.50
    K = np.array([5_800.0, 6_500.0, 7_000.0])

    c = np.asarray(
        bsm.charm(S, K, tau, sigma, r=0.05, option_type=np.array(["C", "C", "C"])),
        dtype=float,
    )
