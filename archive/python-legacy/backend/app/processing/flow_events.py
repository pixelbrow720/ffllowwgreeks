...
def detect_flow_events(
    ...
    *,
    ...
    #
    # DR-4: include ``seq`` in the dedup key when the column is present.
    # The legacy tuple ``(ts, contract, side, size, price, exchange)``
    # collapsed legitimate distinct prints that happened to share every
    # attribute except sequence number — most visible inside multi-leg
    # sweep clusters where two legs hit the same venue at the same
    # microsecond. NULL seq matches still collapse (matches the original
    # behaviour for sources that don't carry a sequence number).
    dedup_cols = [
        c
        for c in (
            "ts", "symbol", "expiration", "strike", "option_type",
            "price", "size", "side", "exchange", "seq",
        )
