...
# Rev 10 BC-11: HIRO bucket vocabulary alignment.
#
# The snapshot endpoint persists buckets named ``1min`` / ``5min`` /
# ``15min`` (pandas-style, canonical). This endpoint historically accepted
# and returned ``1m`` / ``5m`` / ``15m``. We now accept BOTH on input
# (back-compat with old query strings) and ALWAYS return the canonical
# form on the ``bucket`` response field, so both surfaces share one
# vocabulary. See ``API_POLICY.md`` § 1 (additive, non-breaking).
_BUCKET_INPUT_TO_CANONICAL: dict[str, str] = {
    "1m": "1min",
    "5m": "5min",
    "15m": "15min",
    "1min": "1min",
...
_BUCKET_QUERY_PATTERN = "^(1m|5m|15m|1min|5min|15min)$"
...
async def get_hiro(
    ...
    bucket: str = Query("1min", pattern=_BUCKET_QUERY_PATTERN),
    since: datetime | None = Query(
        default=None,
        description=(
            "Start of the window. Defaults to ``now - 1h``. Values older "
            "than 24 hours or in the future return HTTP 400 (Rev 12 BC-19) "
            "— the persisted HIRO buffer never extends further back than "
            "24h, so out-of-range requests are rejected eagerly."
        ),
    ),
    ...
    The persisted series uses 1-minute buckets; ``5min`` / ``15min`` are
    re-aggregated server-side. ``since`` defaults to the last 1 hour;
    out-of-range values return HTTP 400 (Rev 12 BC-19, no silent clamp).

    The ``bucket`` query parameter accepts both the canonical
    ``1min`` / ``5min`` / ``15min`` vocabulary and the legacy
    ``1m`` / ``5m`` / ``15m`` shorthand (Rev 10 BC-11). The response
    ``bucket`` field always returns the canonical form so consumers
    that prime from ``/v1/{symbol}/snapshot`` and ``/v1/{symbol}/hiro``
    see the same value.
    """
    sym_u = symbol.upper()
    target_bucket = _BUCKET_INPUT_TO_CANONICAL[bucket]
    ...
    # REV12-V2: wrap envelope. /v1/{symbol}/hiro is intentionally NOT
    # wrapped in the canonical ``{symbol, computed_at,
    # next_update_in_seconds, data}`` envelope (``API_POLICY.md`` § 1 —
    # "v1 stability exemption"). /v2/ MUST unify all response envelopes;
    # preserve the bare shape here for v1 back-compat.
    return {
        "symbol": sym_u,
        "bucket": target_bucket,
        "since": since.isoformat(),
