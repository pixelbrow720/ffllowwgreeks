...
def upgrade() -> None:
    ...
    # Rev 12 MIG-16: ``LIKE 'GEX_0DTE%%'`` had a configparser-style escape
    # (``%%`` → ``%``) which was never needed — alembic loads migration
    # modules as Python source, not via configparser, and ``sa.text()``
    # does not apply %-escaping. The original ``%%`` survived because
    # neither ``op.execute(plain_string)`` nor ``sa.text(...)`` interprets
    # ``%`` specially in DDL, but the result was inconsistent with
    # ``models.py``'s single-``%`` declaration of the same partial index.
    # Using ``sa.text()`` here makes the binding-style boundary explicit
    # and matches the canonical models.py predicate.
    op.execute(
        sa.text(
            """
